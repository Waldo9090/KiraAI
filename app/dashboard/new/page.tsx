...
export default function NewChatPage() {
  ...
  return (
    // Outermost div handles centering - Add h-full
    <div className="flex h-full flex-grow flex-col items-center justify-center p-6">
          ...
          <div className="relative w-full shadow-sm rounded-xl border bg-white">
            ...
            <div className="flex items-center justify-between p-3 border-t">
               ...
               <div className="flex items-center space-x-4">
                  ...
                  <div className="flex items-center space-x-1 px-2 py-1 rounded-full hover:bg-gray-100 cursor-pointer">
                     <Globe className="h-5 w-5 text-blue-500" /> <span className="text-sm">Web</span>
                     {/* Removed dropdown SVG */}
                  </div>
                  <div className="flex items-center space-x-1 px-2 py-1 rounded-full hover:bg-gray-100 cursor-pointer">
                     <Sparkles className="h-5 w-5 text-blue-500"/> <span className="text-sm">GPT 4o Mini</span>
                  </div>
               </div>
               {/* Right Controls */}
               <div className="flex items-center space-x-4">
                 <div className="flex items-center space-x-2">
                   {/* Using Settings2 icon as placeholder for Merlin Magic */} 
                   <Settings2 className="h-5 w-5 text-gray-500"/>
                   <span className="text-sm text-gray-600">Kira Magic</span>
                   {/* Placeholder Switch */} 
                   <div className="w-10 h-5 bg-gray-200 rounded-full relative cursor-pointer"><div className="absolute inset-y-0 left-0 w-5 h-5 bg-white rounded-full shadow"></div></div>
                 </div>
                 <Button onClick={handleSend} size="icon" className="bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-full w-8 h-8" disabled={isLoading}>
